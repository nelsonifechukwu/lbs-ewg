...
2. **Snapshot on every `./bin/dev` start** — automatic, no setup. Copies live to
...
```bash
# Stop bin/dev first (Ctrl+C).
./bin/restore                                 # interactive picker
./bin/restore backups/lbs.db.bak-20260513-115632
```

### Remove everything

```bash
./bin/uninstall-protection                    # removes hook + launchd job
```

Snapshots in `backups/` are never deleted by uninstall — only by the >30-day
prune in `bin/dev` and the launchd job.
